<?php
    ...
    $query="select * from reviews where PackageOrAgencyId='".$agencyId."'";
...
while($feedItem = mysqli_fetch_assoc($ret))
{
	...
	if($feedItem["ReviewTypeId"]==1){
		$for="For Agency";
	}
	else{
		$packageId=$feedItem['PackageOrAgencyId'];
		$query2="select * from package where PackageId='".$packageId."'";
    $ret2=mysqli_query($connection,$query2);
	$feedItem2 = mysqli_fetch_assoc($ret2);
		$for="For ".$feedItem2['Name'];
	}
	...
	echo "<div class='for'><b>" . $for . "</b></div><br>";
